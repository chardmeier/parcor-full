...
The phrases \textsl{ce qui}, \textsl{ce que} and \textsl{ce dont} should be
marked up as a single markable, annotated as relative pronouns and linked to an
appropriate antecedent when they are used to introduce relative clauses, such as
in example~\Next[a]. When they are used as interrogative pronouns, such as in
example~\Next[b], they should not be annotated.

\ex.
\a. \textsl{Le docteur m'a dit que j'étais guéri, [ce qui] m'a surpris.}
\b. \textsl{Mais comment peux-tu décider d'avoir des enfants alors que tu es en
train d'étudier \textbf{tout ce qui} peut mal se passer ?}

In example~\Last[a], the antecedent could be either \textsl{Le docteur m'a dit
que j'étais guéri} or just \textsl{j'étais guéri}, depending on what causes the
surprise. This must be resolved from the context.
